...
%             fname='C:\Program Files (x86)\Default Company Name\Setup1\FetalSim_log';
            ...
            if ( fid ~= -1 )
                while ( ~feof( fid ) )
                    ...
                    elseif strcmp(str, 'r_lung_activity'                ) fscanf(fid,'%s',1); MRX.Par.act.r_lung_activity       = fscanf( fid,'%f',1);
                    elseif strcmp(str, 'l_lung_activity'                ) fscanf(fid,'%s',1); MRX.Par.act.l_lung_activity       = fscanf( fid,'%f',1);
                    ...
                    elseif strcmp(str, 'airway_activity'                ) fscanf(fid,'%s',1); MRX.Par.act.airway_activity       = fscanf( fid,'%f',1);
...
%                 img  = img(xdim,ydim,zdim);
        ...
        function [img,nois] = addNoise(MRX, img)
            ...
            nois  = zeros(size(img));%stdev*complex(randn(size(img)),randn(size(img)));
            img   = img + nois;            
        end
