...
classdef MRXCAT_CMR_CINE < MRXCAT
    ...
    methods
        function [MRX, image] = MRXCAT_CMR_CINE( varargin )
...
%             sen = MRX.calculateCoilMaps; sen = single(sen);
            %             load('smaps_phantom','smaps')
            %             sen=zeros(256,304,1,8);
            %             for loop=1:8
            %             sen(:,:,1,loop)=imresize(smaps(:,:,loop),[256,304]);
            %             end
...
%             MRX.Par.scan.noisestd = MRX.computeNoiseStdDev( sen );
            ...
            % --------------------------------------------------------------------
            for t=1:MRX.Par.scan.frames % heart phases
...
%                 [img,noi] = MRX.addNoise(img);
        ...
        function stdev = computeNoiseStdDev( MRX, sen )
            ...
            for t=MRX.Par.scan.phases
                ...
                [img,msk]   = MRX.mapTissueProps(data); img = single(img); msk = single(msk);
                img         = MRX.multiplyCoilMaps(img,sen);
                roi=msk==MRX.Par.act.bldplLV_act|msk==MRX.Par.act.bldplRV_act|msk==MRX.Par.act.bldplLA_act|msk==MRX.Par.act.bldplRA_act;
                ...
                roialls(:,:,:,:,t) = repmat(roi,[1,1,1,size(sen,4)]);
            end
            save('MRX_Noise_ROI','roialls');
        ...
        function [img,msk] = mapTissueProps(MRX, data)
            ...
            for i=1:length(act)
                ...
                switch char(tis(i))
                    ...
                    case {'muscle_activity','lung_activity'}
                        rho = MRX.Par.tissue.rhomuscle;
                        t1  = MRX.Par.tissue.t1muscle;
                        t2  = MRX.Par.tissue.t2muscle;
                    case 'liver_activity'
                ...
                % ----------------------------------------------------------------
                %sig = rho*sin(a)*(1-exp(-tr/t1))/(1-cos(a)*exp(-tr/t1))*exp(-te/t2star);
