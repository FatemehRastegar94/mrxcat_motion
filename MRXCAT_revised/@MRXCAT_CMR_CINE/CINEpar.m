...
%   MRXCAT settings
% --------------------------------------------------------------------
% % (Updated) - reference: T1, T2 Relaxation and Magnetization Transfer in Tissue at 3T, Stanisz, MRM 2005
% % B0 = 3T
% T2liver = 42;
% T2muscle = 50;
% T2Heart = 47;
...
RhoLiver    = 30.0;   %90                      % Proton density liver  [%]
RhoBone     = 12.0;                         % Proton density bone   [%]

T1Muscle    = 900.0;                        % T1 muscle             [ms]
T1Fat       = 350.0;                        % T1 fat                [ms]
T1Blood     = 1200.0;                       % T1 blood              [ms]
T1Liver     = 300.0;  %800                      % T1 liver              [ms]
T1Bone      = 250.0;                        % T1 bone               [ms]

T2Muscle    = 50.0;                         % T2 muscle             [ms]
T2Fat       = 30.0;                         % T2 fat                [ms]
T2Blood     = 100.0;                        % T2 blood              [ms]
T2Liver     = 30.0;  %50                       % T2 liver              [ms]
T2Bone      = 20.0;                         % T2 bone               [ms]

TR          = 3.0;                          % Repetition time       [ms]
TE          = 1.5;                          % Echo time             [ms]
Flip        = 56.0;                         % Flip                  [deg]
Frames      = 0;                           % Number of heart phases (default: 24; 0=use # XCAT frames (.bin files))
Segments    = 1;                            % Number of segments

BoundingBox = [64/383,320/383;16/383,320/383;0,1];%[0.2,0.6;0.3,0.7;0.0,1.0];    % BoundingBox in rel. units
RotationXYZ = [0,0,0];%[115.0;35.0;240.0];           % Rotations about x,y,z [deg]  (default: 115/35/240)
                                            % x=(RL) y=(AP) z=(FH)
LowPassFilt = 1;                            % low-pass filter images
FilterStr   = 1.2;                          % low-pass filter strength (default: 1.2)

SNR         = 40;                           % signal-to-noise ratio
Coils       = 1;                            % number coils (Biot-Savart)
CoilDist    = 320;                          % body radius           [mm]    = distance of coil centres from origin
...
if Frames>0 % only overwrite Par.scan.frames, if Frames ~= 0
    ...
    frames_max              = MRX.Par.scan.frames_xcat;
